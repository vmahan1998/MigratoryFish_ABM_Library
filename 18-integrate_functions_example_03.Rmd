...
The table below shows the full dependency chain for an alewife in the landward migration phase. Functions are listed in the order they must be called each tick. See Appendix C of Mahan (2025) for the complete equation set for each submodel.
...
The P-MEM produces outputs across three categories, all of which are visualized in the Mercury Risk Explorer (<https://vkzfr3-vanessa-mahan.shinyapps.io/Mercury_Risk_Explorer/>):

-   **Migration timing and passage counts:** Compare `alewives-on-line-daily-total-landward` against the 2023 Milford Dam fish lift counts (the model evaluation dataset used in Mahan 2025, Table 4.3). The stochastic migration probability ramp should produce a realistic spread of entry dates peaked in late April–May.
...
-   **Exposure pathway dominance:** Compare `mean [hg-uptake-risk] of alewives` (branchial, gill-contact exposure) against `mean [hg-foraging-total] of alewives` (particulate ingestion) across predation scenarios. Mahan (2025, Chapter 4) shows that predation pressure shifts which pathway dominates individual cumulative burden.
